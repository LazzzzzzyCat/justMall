...
import com.justmall.common.constant.WareConstant;
...
import com.justmall.ware.entity.vo.item;
...
import com.justmall.ware.dao.PurchaseDao;
...
public class PurchaseServiceImpl extends ServiceImpl<PurchaseDao, PurchaseEntity> implements PurchaseService {
    ...
    public void receivePurchase(List<Long> ids) {
        // 查询未分配或新建的采购单
        List<PurchaseEntity> purchaseEntityList = ids.stream().map(id -> baseMapper.selectById(id)).filter(item -> (item.getStatus() == WareConstant.PurchaseStatusEnum.CREATE.getCode() || item.getStatus() == WareConstant.PurchaseStatusEnum.ASSIGNED.getCode())).peek(item -> item.setStatus(WareConstant.PurchaseStatusEnum.RECEIVE.getCode())).collect(Collectors.toList());
        // 修改
